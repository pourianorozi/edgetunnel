...
let 面板地址 = 'https://edt-pages.github.io';
const 配置缓存 = new Map(), 配置缓存TTL = 30000;
function 清除配置缓存(userID) { if (!userID) { 配置缓存.clear(); return; } for (const k of 配置缓存.keys()) if (k.startsWith(userID)) 配置缓存.delete(k); }
async function 获取面板页面(path) {//获取并缓存面板静态页面
    const url = 面板地址 + path;
    try {
        ...
        if (resp && resp.ok) {
            ...
            await cache.put(cacheKey, new Response(cloned.body, { status: resp.status, statusText: resp.statusText, headers }));
        }
        return resp;
    } catch (e) {
        console.error('[面板页面]', e);
        return fetch(url);
    }
}
async function 读取config_JSON缓存(env, hostname, userID, 重置配置 = false) {//KV读取缓存加速
    const now = Date.now();
    const 键 = userID + '@' + hostname;
    if (重置配置) { 清除配置缓存(userID); return await 读取config_JSON(env, hostname, userID, true); }
    const 命中 = 配置缓存.get(键);
    if (命中 && now - 命中.t < 配置缓存TTL) return 命中.cfg;
    const cfg = await 读取config_JSON(env, hostname, userID);
    配置缓存.set(键, { t: now, cfg });
    return cfg;
}
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (env.PANEL || env.PAGES_URL) 面板地址 = String(env.PANEL || env.PAGES_URL).replace(/\/+$/, '');
        ...
        if (管理员密码 && upgradeHeader === 'websocket') {// WebSocket代理
            ...
            if (!管理员密码) return 获取面板页面('/noADMIN').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }); });
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (访问路径 === 'admin/log.json') {// 读取日志内容
                        ...
                        try {
                            ...
                            const errorResponse = { msg: 'بررسی آمار درخواست‌ها ناموفق بود؛ دلیل: ' + err.message, error: err.message };
                        ...
                        if (url.searchParams.get('url')) {
                            ...
                            try {
                                ...
                                const errorResponse = { msg: 'تأیید API بهینه‌سازی ناموفق بود؛ دلیل: ' + err.message, error: err.message };
                                return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            }
                        }
                        ...
                        if (url.searchParams.has('socks5')) {
                            ...
                            return new Response(JSON.stringify({ error: 'پارامتر پروکسی ارسال نشده است' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
                        return new Response(JSON.stringify(检测代理响应, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }

                    config_JSON = await 读取config_JSON缓存(env, host, userID);

                    if (访问路径 === 'admin/init') {// 重置配置为默认值
                        try {
                            config_JSON = await 读取config_JSON缓存(env, host, userID, true);
                            ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Init_Config', config_JSON));
                            config_JSON.init = 'پیکربندی به پیش‌فرض بازنشانی شد';
                            return new Response(JSON.stringify(config_JSON, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        } catch (err) {
                            const errorResponse = { msg: 'بازنشانی پیکربندی ناموفق بود؛ دلیل: ' + err.message, error: err.message };
                            return new Response(JSON.stringify(errorResponse, null, 2), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
                        ...
                        if (访问路径 === 'admin/config.json') { // 保存config.json配置
                            try {
                                ...
                                // 验证配置完整性
                                if (!newConfig.UUID || !newConfig.HOST) return new Response(JSON.stringify({ error: 'پیکربندی ناقص است' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                                ...
                                console.error('ذخیره‌سازی پیکربندی ناموفق بود:', error);
                                return new Response(JSON.stringify({ error: 'ذخیره‌سازی پیکربندی ناموفق بود: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            }
                            ...
                            try {
                                ...
                                if (!newConfig.init || newConfig.init !== true) {
                                    if (newConfig.Email && newConfig.GlobalAPIKey) {
                                        ...
                                        return new Response(JSON.stringify({ error: 'پیکربندی ناقص است' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                                    }
                                }
                            ...
                            try {
                                ...
                                if (newConfig.init && newConfig.init === true) {
                                    ...
                                    if (!newConfig.BotToken || !newConfig.ChatID) return new Response(JSON.stringify({ error: 'پیکربندی ناقص است' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            ...
                            try {
                                ...
                                清除配置缓存(userID);
                                return new Response(JSON.stringify({ success: true, message: 'آی‌پی‌های سفارشی ذخیره شدند' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            } catch (error) {
                                console.error('ذخیره‌سازی آی‌پی‌های سفارشی ناموفق بود:', error);
                                return new Response(JSON.stringify({ error: 'ذخیره‌سازی آی‌پی‌های سفارشی ناموفق بود: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            }
                        } else return new Response(JSON.stringify({ error: 'مسیر POST پشتیبانی‌نشده' }), { status: 404, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
                        ...
                        if (订阅类型 === 'mixed') {
                            ...
                            try {
                                ...
                                } else return new Response('خطای سرویس تبدیل سابسکریپشن: ' + response.statusText, { status: response.status });
                            } catch (error) {
                                return new Response('خطای سرویس تبدیل سابسکریپشن: ' + error.message, { status: 403 });
                            }
                        }
            ...
            } else if (!envUUID) return 获取面板页面('/noKV').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }); });
        }
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数) {
    ...
    try {
        ...
        if (!tcpSocket) return { success: false, error: 'اتصال به سرور پروکسی ممکن نشد', proxy: 代理协议 + "://" + 完整代理参数, responseTime: Date.now() - startTime };
        try {
